...
router.post("/", async (req, res) => {
  ...
  if (!title || !link || !link.name || !link.url)
    return res.status(400).json({ error: "Missing required fields" });

  // INSERT into the resources table
  const resourceQueryString = `INSERT INTO resources (author_id, title, description, created_at)
  VALUES ($1, $2, $3, NOW())
  RETURNING *;
  `;

  // INSERT into the resource_links table
  const linkQueryString = `INSERT INTO resource_links (resource_id, name, url, description)
  VALUES ($1, $2, $3, $4)
  RETURNING *;
  `;

  // INSERT into resource_topics table
  const topicQueryString = ` INSERT INTO resource_topics (resource_id, topic_id)
  VALUES ($1, $2)
  RETURNING *;
  `;
...
router.patch("/:id", async (req, res) => {
  ...
  const editResourceQueryString = `UPDATE resources
  SET title = $1,
  description = $2
  WHERE id = $3
  RETURNING *;
  ...
  const editResourceLinksQueryString = `UPDATE resource_links
  SET name = $1,
  url = $2,
  description = $3
  WHERE resource_id = $4
  ...
  const resourceOwnerQueryString = ` SELECT *
  FROM resources
  ...
  try {
    ...
    const resourceOwnerQueryResult = await db.query(
      resourceOwnerQueryString,
      resourceOwnerQueryStringValues
    );
    ...
    if (resourceOwnerQueryResult.rows.length === 0)
      return res
        .status(403)
        .json({ error: "Unauthorized access to edit this resource" });

    // if it's successful, proceed to update.
    const editResourceQueryValues = [title, description, resource_id];
    const updateResourceResult = await db.query(
      editResourceQueryString,
      editResourceQueryValues
    );

    // update the link, too.
    const editResourceLinksQueryValues = [
      link.name,
      link.url,
      link.description,
      resource_id,
    ];
    const updateLinkResult = await db.query(
      editResourceLinksQueryString,
      editResourceLinksQueryValues
    );
...
router.delete("/:id", async (req, res) => {
  ...
  WHERE id = $1 AND author_id = $2
  `;
  const deleteResourceQueryString = `DELETE FROM resources
  WHERE id = $1
  RETURNING *
  `;
  ...
  try {
    ...
    const resourceOwnerQueryResult = await db.query(
      resourceOwnerQueryString,
      resourceOwnerQueryValues
    );

    if (resourceOwnerQueryResult.rows.length === 0)
      return res
        .status(403)
        .json({ error: "Unauthorized access to delete this resource" });

    // if owner
    const deletedResource = await db.query(deleteResourceQueryString, [
      resource_id,
    ]);
